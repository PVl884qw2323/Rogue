...
//Рисование вокруг игрока, основывается на длине и ширине
...
CanvasDisplay.prototype.drawPlayer= function(dirNum){
	this.context.drawImage(playerSprite,0,0,50,50,this.shownWidth/2*this.canvas.width/10, this.shownLength/2*this.canvas.height/10,this.canvas.width/10,this.canvas.height/10);
};

//Изменения позиции противника. Changes direction when obstacle is encountered.
...
//Назначает горизонтальные коридоры для соединения комнат.
//Демоны назначаются на некоторые из этих коридоров.
...
//Образуются фиксированные вертикальные коридоры.
...
//Создаются комнаты и прокладываются коридоры, соединяющие их.
